...
# 用户配置区
# =========================================================
INPUT_LAS = r"E:\Homework\激光雷达\City.las"
OUTPUT_DIR = r"E:\Homework\激光雷达\CSF_Output01"
...
# 是否自动打开 HTML（如果你不想自动打开，可以改成 False）
...
# ---------- CSF 参数（按论文） ----------
CSF_GR = 0.5                  # grid resolution
CSF_DT = 0.35                 # time step（比 0.65 更稳）
CSF_RI = 2                    # rigidness: 1/2/3
CSF_ST = True                 # steep slope fit
CSF_HCC = 0.5                 # classification threshold
CSF_HCP = 0.3                 # post-process threshold
CSF_MAX_ITER = 1000
CSF_STOP_THRESHOLD = 0.01
CSF_INITIAL_HEIGHT_OFFSET = 2.0
...
def visualize_final_csf_result(full_xyz, ground_mask, nonground_mask,
    ...
    if ground_vis.shape[0] > 0:
        pcd_ground = make_o3d_pcd(ground_vis, color=[0.76, 0.59, 0.25])   # 棕色
        geoms.append(pcd_ground)

    if nonground_vis.shape[0] > 0:
        pcd_nonground = make_o3d_pcd(nonground_vis, color=[0.20, 0.45, 0.85])  # 蓝色
...
def main():
    ...
    print("=" * 90, flush=True)
    print("基于论文 CSF 的 LAS 点云地面滤波程序（Plotly 2D/3D + Open3D）", flush=True)
    print("=" * 90, flush=True)
